...
def get_snp_nvidia_qemu_cmd(resource: VMResource, config: dict) -> List[str]:
    vmconfig: VMConfig = get_vm_config("snp")
    ssh_port = config["ssh_port"]
    if config["boot_prealloc"]:
        prealloc = "on"
    ...
    try:         
        with open("/sys/bus/pci/drivers/vfio-pci/new_id", "a") as f:
...
def start(
    ...
    if type == "amd":
        ...
        if direct:
            ...
            qemu_cmd = get_snp_nvidia_qemu_cmd(resource, config)
