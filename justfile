...
QEMU_COCONUT := join(BUILD_DIR, "qemu-coconut-igvm/bin/qemu-system-x86_64")
OVMF_COCONUT := join(BUILD_DIR, "ovmf-coconut-fd/FV/OVMF.fd")
...
# Tested with coconut-qemu 8.2.0
start-snp-direct-coconut:
    sudo {{QEMU_COCONUT}} \
        -cpu EPYC-v4,host-phys-bits=true \
        -smp {{smp}} \
        -m {{mem}} \
