...
for i in $(seq 1 $repeat); do
    ...
    inv vm.start $([[ $remote == true ]] && echo "--remote") --virtio-nic --virtio-nic-vhost --action=${action} --virtio-iommu --extra-cmdline swiotlb=524288,force
    ...
    inv vm.start $([[ $remote == true ]] && echo "--remote") --type snp --virtio-nic --virtio-nic-vhost --action=${action} --extra-cmdline idle=poll --name-extra -poll
    ...
    inv vm.start $([[ $remote == true ]] && echo "--remote") --type snp --virtio-nic --virtio-nic-vhost --action=${action} --extra-cmdline cpuidle_haltpoll.force=Y --name-extra -hpoll
done
